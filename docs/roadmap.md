...
| 2A.3 | Risk-tier aware drafts | PR templates respect CODEOWNERS / change tiers documented in knowledge corpus — **done** (`risk_tiers.py`, PR template, scaffold workflow body) |
| 2A.4 | Audit log | Prompt, retrieval sources, tool calls, confirm events persisted and queryable — **done** (`audit_log.py`, [audit-log.md](../docs/audit-log.md)) |
...
| 2D.3 | Injection defenses | Tool allow-list, output moderation, kill switch documented and tested — **done** ([injection-defenses.md](../docs/injection-defenses.md)) |
...
1. Phase **2D.4** Citation-required mode — LLM answers rejected in eval if sources missing

Phase **1** workstreams **1A–1D** and **2A–2C** are on `main`. Phase **2D.1–2D.3** (ABAC, groundedness eval, injection defenses) ship with this doc.
