...
**Alternative (Docker Compose only):** from the repo root, the same portal stack is `docker compose up --build -d` (**3000** web, **8080** API). For **portal + Backstage**, use `docker compose --profile backstage up --build -d` (**3001** + **7007** as well), `make up-all`, or set `COMPOSE_PROFILES=backstage` in `.env` — port map in [docs/local-compose.md](docs/local-compose.md#local-ports-docker-compose). Make remains the documented default for ingest, CI, and host Backstage dev.
